		...
		<tr><td>${num}<c:set var="num" value="${num - 1}"></c:set> </td>
			...
			<!-- title은 풍선 도움말 또는 미리 보기 -->
				<td title="${board.content }">
					...
					<a href="content.do?num=${board.num }&pageNum=${currentPage}&id=${id}" class="list1">
						${board.subject }</a>
...
<div align="center">
	...
	<c:if test="${startPage - PAGE_PER_BLOCK > 0 }">
		<button onclick="location.href='list.do?pageNum=${startPage-1}'" class="btn4">이전</button>
	</c:if>
	<c:forEach var="i" begin="${startPage}" end="${endPage}">
		<button onclick="location.href='list.do?pageNum=${i}'" class="btn4">${i}</button>
	</c:forEach>
	<!-- 아직 보여줄 것이 남아 있다 -->
	<c:if test="${endPage - totalPage < 0 }">
		<button onclick="location.href='list.do?pageNum=${endPage+1}'" class="btn4">다음</button>
	</c:if>
	<p><button onclick="location.href='writeForm.do?num=0&pageNum=1&id=${id}'" style="float: right; margin-right: 50px;" class="btn5">글쓰기</button>
	<br><br><p>
	<form class="table-form" action="list.do">
			...
			<input class="btn4" type="submit" value="검색">	<!-- style="color: white; background: darkorange;" --> 
	</form>
</div>
</div>
